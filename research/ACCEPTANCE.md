...
> 本文是**按时间累积**的验收日志：逐节记录当时真实执行的命令与输出，后续轮次只在文末追加。
> 因此文中出现的测试数量、状态目录名、能力清单（例如早期的「回收站 / 恢复」）都是**那一轮**的
> 状态，不代表当前实现 —— 当前状态请以 `README.md` 与代码为准。
...
## 26. 一次自己吓自己的"严重 bug"，以及它真正的结论

Round 14 想验证一条**潜在**的灾难路径：`recordFor` 在文件读不出来时给出的记录是
`{ description: '', loadable: false, invocationPolicyValid: true }`，而 `overridesOf()` 只按
`invocationPolicyValid` 过滤 —— 于是这会变成一条**描述为空**的候选。注册表的 `validateCandidate`
对空描述是**抛异常**的：

```
skill provider "..." returned skill "x" without a description
```

一条候选不合法 → 整次快照报废 → 模型一条技能都拿不到。场景还不罕见：Windows 上文件被杀软或
编辑器短暂占用，正是"被占用、已删除或不是文本"那一类。

### 实测结果：快照是空的 —— 但那是我自己的测试写错了

第一次跑，快照返回**空清单**，连健康的那条技能也不见了。看起来完全坐实了。

然后照着规矩回头看：**把修复撤掉，测试还应该失败吗？** 撤掉之后 —— **照样通过**。

原因在测试自己身上：我写的是 `createScope(root, {})` 建作用域，却拿一个新的 `{}` 去
`snapshot({ scope })`。两个对象不是同一个 key，作用域对不上，快照当然是空的 —— 跟候选合不合法
一点关系都没有。`test/layers.test.mjs` 里用的是同一个 `key` 变量，我照抄时漏了。

### 真正的结论

拿修正后的测试重跑，**撤掉修复也通过** —— 因为注册表对提供方抛出的异常是**按提供方隔开**的，
我们的候选不合法只影响我们这一路，别的技能照常出现。所以：

> 这**不是**一个已复现的 bug 的修复，是**防御性加固**。

两道筛选（`overridesOf` + `list()`）保留，守的是"覆盖提供方永远不发不合法候选"这条不变量 ——
代价是零，收益是万一哪天注册表的容错策略变了，我们不会变成那个把整份清单拖垮的人。
`test/overlay-robustness.test.mjs` 作为不变量测试保留，并在注释里写明它守的是什么、以及
"这不是已复现 bug 的修复"。
...
## 28. 布局重做：从"两栏挤压"到"整宽卡片"

用户的评价很直接：布局太差，学学别的页面。对着截图一比，问题一眼就出来了 ——

**我把技能列表塞进了 `minmax(260px,340px)` 的窄栏**，于是名字被截成 `apple-liquid-gl...`、
描述被截成 `Build Apple-grade UI ...`、路径被截成 `C:\Users\Administra...`。而横向空间本来就够：
那是个 `minmax(0,1fr)` + 340px 的两栏网格，白白把一半宽度让给了一个**多半时候是空的**详情栏。

对照「插件」「模型」「Agent 预设」三页，它们的语言是一致的：

- **整宽卡片**：标题 + 描述在左，操作在右，横向空间给足；
- **单列纵排**，不分主次栏；
- 添加入口是**整宽的虚线按钮**（「模型」页的"＋ 添加提供方"、「Agent 预设」的"＋ 用…创建…"）。

### 改法

1. **拆掉两栏网格**，整宽单列。列表拿到全部宽度，名字、描述、路径都不再被压。
2. **详情长在卡片里**：点卡片在**卡片内部**展开（虚线分隔 + 平台模块底色做嵌面），
   而不是挤到旁边一栏去。再点一次收起。
3. **添加入口下移**，改成整宽虚线按钮 `＋ 新建技能` / `＋ 导入技能`，照「模型」页那一行
   （两列定宽网格，不用 flex-wrap —— 换行阈值不该随窗口差几个像素就变）。
4. 卡片标题改用正文黑体（`font-family:inherit`），等宽只留给路径这类真正需要对齐的东西。
5. 描述给两行而不是截成一行 —— 卡片有空间了。

### 顺手抓到的一个坑

CSS 注释里写了**反引号**（`.dshsm-row__meta` 那种写法），而 CSS 整个是一个**模板字符串** ——
反引号把字符串提前截断了。`node --check` **看不出来**（截断后的代码在语法上居然合法），
是渲染测试报 `row__meta is not defined` 才暴露的。

### 验收：截图 + 计算样式 + 全流程演练

给探针加了 `--shot <path>`，直接 `Page.captureScreenshot` 存 PNG —— 前面几轮全靠计算样式和 DOM
判断，**我一次都没亲眼看过**，这次补上。（写的时候踩了一下：`Cdp#receive` resolve 的是
`message.result` 这个 payload 本身，不是整条消息，所以取 `result.data` 而不是 `result.result.data`。）

真实浏览器里一次跑完 **--styles --exercise --exercise-import**：

- 新建 → 编辑 → 删除进回收站 → 恢复 → 永久删除：**通过**，磁盘零残留；
- 导入 ZIP / 单个 Markdown / 按路径：**三条分支全部通过**；
- 30 项计算样式与提示词页逐项一致：**全过**；
- 页面无 JS 报错。

期间修了三处**验收工具自身**的假设（都不是产品 bug）：卡片改成可切换后 `selectRow()` 会误收起详情、
导入入口只在技能页渲染而演练结束时停在回收站页、样式取样前没切回技能页导致取到非选中态的颜色。
按规矩，这三处都改成"表达意图"（确保详情是打开的 / 先切回技能页），而不是硬编码时序。

## 29. 按用户要求砍掉搜索与回收站，并把按钮对齐到提示词页

用户看着界面给了两条指令：**去掉搜索和回收站的逻辑**，**按钮样式改成提示词页那样**。

### 回收站：整套移除，删除 = 永久删除

这不是改文案，是把一条已经验收过的能力拿掉，所以先确认了一次才动手。确认结果是彻底删除，
于是回收站从四个层次一起消失：

- `lib/operations.js`：`moveToTrash` / `listTrash` / `restoreFromTrash` / `purgeFromTrash` /
  `findTrashMeta` / `trashRoot` / `trashId` / `movePath` 全部删除，换成一个 `deleteSkill`。
  顺带变成死代码的 `cpSync`、`renameSync`、`statSync`、`storeDir` 导入也一并收掉（399 → 212 行）。
- `lib/routes.js`：`/skill/trash`、`/trash/restore`、`/trash/purge` 三个端点换成一个
  `/skill/delete`；`/catalog` 响应里不再有 `trash` 字段。
- `lib/tools.js`：`skills_delete` 的说明从"移进回收站、可恢复"改成"永久删除、不可撤销"。
- `client/client.js`：`TrashPanel`、标签行、搜索框、`tab`/`query` 两个 state 全部删除。

**永久删除必须问一句。** 点一下直接抹掉别人的文件是不可接受的，所以卡片里的删除按钮
是就地二段式的：第一下只进入确认态（"永久删除 X？不可撤销。"），第二下才真删。
不用 `window.confirm` —— 那个会阻塞宿主线程，也没法在渲染测试里断言。
`test/fullstack.test.mjs` 里专门钉住了这一点：**确认之前文件必须还在，且一个请求都不许发出**。

### 顺手抓到自己埋的两个雷

1. **`createSkill` 被我一起切掉了。** 用 python 按标记切段时，`trashRoot` 到 `findTrashMeta`
   这一段**跨过了 `createSkill` 和 `writeSkillContent`** —— 两个函数直接消失。测试立刻报
   `does not provide an export named 'createSkill'`，从 git 取回补上。
   教训：按标记切段之前，先确认这段里**没有别的东西**。
2. **审计脚本自己的键冲突。** 我给样式审计加"开关"和"图标按钮"两对时都用了 `kind: 'control'`，
   而 `out[kind]` 是用 kind 做键的 —— 后一对（`.dshsm-btn`，卡片未展开时为空）把前一对
   （`.dshsm-switch`，确实存在）**盖掉了**，于是报告"我们 缺"。改成按**标签**做键。
   这个 bug 的表现形式很坏：它让审计**指控一个不存在的问题**。

### 按钮：量出来再改，不猜

"改成提示词页那样"是个模糊说法，所以直接在浏览器里量了提示词页实际渲染出来的控件 ——
先按标签做键修好了取样，才拿到可信数字：

| 项 | 我们 | 提示词页 | 结论 |
|---|---|---|---|
| 开关 width / height | 36px / 20px | 36px / 20px | 已经一致 |
| 开关 border-width | 0px | 0px | 已经一致 |
| 开关打开态背景 | `rgb(34,197,94)` 绿 | `rgb(15,17,21)` 近黑 | **真正的差别，改** |
| 圆角 | `999px` | `10px` | 20px 高下都是全圆，等价，不参与对比 |
| padding-left | 0px | 2px | 圆钮用绝对定位 vs padding 内缩，等价，不参与对比 |

所以按钮这块**真正要改的只有一处**：打开态不再用绿色，改用主文字色（深色药丸 + 反色圆钮），
和提示词页一致；绿色留给状态圆点。另外给详情里的操作按钮加上前导符号
（`✎ 编辑正文`、`✺ 恢复文件设定`、`✕ 删除`），与提示词页底部那排
"＋ 新增提示词 / ⟳ 订阅来源" 是同一个语言。

把"等价但写法不同"的两项**从对比里去掉而不是硬凑数值** —— 为了让审计变绿而去改
一个视觉上没有任何区别的圆角，是在给未来的自己挖坑。

### 一个只有真机才会暴露的坑

改完路由后真机演练失败：确认删除点下去了，文件还在。原因是 **host 半的生命周期**——
客户端 bundle 每次页面加载都会重新拉，所以按钮是新的；而 `lib/` 是在 `dsh web` 启动时
加载的，那个 3099 实例是在我改路由**之前**起的，它根本不认识 `/skill/delete`。
重启实例后一次通过。

**验收工具验证的产品行为，取决于实例是什么时候起的** —— 改了 host 半就必须重启再验，
否则验的是上一个版本。

### 验收

- `node --test`：**123 项全通过**。
- 真机 `--styles --exercise --exercise-import`：**全部通过** —— 新建、编辑、
...
## 31. 一致性核对的误报：把「我们自己赢得了合并」当成了「DSH 没有」

用户贴出界面上的红字问「红字什么意思」：

> 本插件多报了 7 条（DSH 没有，模型收不到）：apple-liquid-glass、frontend-ui-system、
> grilling、python-typed-development-standards、refactor、reverse-flow、test-driven-development

**这是误报，而且模型一条不少地收到了。** 同一时刻系统的技能目录恰好重新下发了一遍，
列出的正是这 7 条 —— 它们就在模型手里。

### 成因

`lib/divergence.js` 的 `compareWithRegistry` 只统计 `provider === 'filesystem'` 的记录。
但注册表是**合并之后**的视图：同一个技能名只留胜出的那一条。本插件的覆盖层注册在
`OVERLAY_RANK = 0`，而 preset 层的文件系统提供方是 400 —— **rank 越小越优先**，所以
凡是用户手动改过的技能，胜出者都是我们自己的 overlay。

于是这 7 条被"只认 filesystem"的过滤全部丢出注册表一侧，`registry` 从 9 掉到 2，
`missing` 冒出 7 条。真机响应：

```
修复前： {"ours": 9, "registry": 2, "missing": [7 条], "consistent": false}
修复后： {"ours": 9, "registry": 9, "missing": [], "overlaid": 9, "consistent": true}
```

直接证据在同一个响应的宿主层清单里 —— 那 7 条的 `provider` 明明白白是
`"dsh-skills-manager"`，不是 `"filesystem"`。

### 为什么原来的理由站不住

那条过滤的注释写着「本插件的 overlay 也注册在同一个注册表里，把它算进来就成了自己跟
自己比」，测试里还专门钉了一句「overlay 不能算进去，否则永远一致」。**这个前提是错的**：

- 注册表是合并后的视图，它回答的问题就是"模型到底收得到什么"。一条技能由谁供给，
  不改变它到没到模型手里。
- overlay **造不出**技能：`provider.js` 的 `list()` 只对覆盖表里已有、且在自己扫描结果里
  存在（`canEmitCandidate`）的技能发候选，`get()` 加载时还会重新扫描校验一遍。
- 过滤掉它得到的不是"更严格的比对"，而是**把所有被覆盖过的技能都误判成不存在**。

误报比不报更糟：它会让这个提示彻底失去可信度，之后真出问题时没人再看它。而这个提示
存在的全部意义，就是"别说技能在生效而模型根本没收到"。

### 改法

比对对象改成**合并后的完整结果**（所有提供方都算数，即模型真正收到的那一份），
并额外给出 `overlaid` 计数说明有多少条由本插件的覆盖层接管：

```
与 DSH 实际解析一致（9 条，其中 9 条由本插件的覆盖层接管）
```

检测能力没有变弱：若我们的根解析与 `dsh-skill-filesystem` 分叉、某条技能谁都不供给，
它就不在合并结果里 → 照样点名。

### 先红后绿

新增的测试 `被覆盖层接管的技能不算多报 —— 换了胜出者，技能本身还在` 在改代码前**先跑红**，
改完转绿。同时把那条固化错误前提的旧测试改成 `比对的是合并后的结果：别的提供方供给的
技能也算「收得到」`，并在注释里写清为什么推翻它。全套 **126 项通过**（+1）。

真机复刻验证：在 3099 上把 8 条全部启用（复刻用户场景）后，`/registry` 从
`registry: 2 / missing: [7]` 变成 `registry: 9 / missing: [] / overlaid: 9`。

### 插曲：验证时动了用户的态，已还原

为了复刻场景我在 3099 上启用 8 条技能，但 3099 与用户的 3080 **共用同一份 `state.json`**。
其中 `grill-me`、`improve-codebase-architecture` 是用户原本**没启用**的（他那张截图里
这两条没有「手动启用」徽标）。3080 的内存态给出了权威的原始清单（7 条），据此删掉了
我多加的 2 条并验证：重新启动一个实例读到的覆盖恰好是那 7 条。

教训：**这个 $DSH_HOME 下的 `state.json` 是用户真实在用的**，拿它做实验必须先备份、
后还原。备份文件用完已删。

### 另一处语言陷阱

`describeDivergence` 里我写了个中文变量名 `接管`，结果 `const 接管` 落盘时 **`const` 与
标识符之间的空格被吃掉了**，变成 `const接管`，报 `ReferenceError: const接管 is not defined`。
**代码里不要用中文标识符** —— 注释和字符串随便用，标识符老老实实 ASCII。
