 ...
 * 本插件的候选**不写 `metadata`**。
 *
 * 曾经在这里塞一个 `dshSkillsManager: {...}` 的标记，后来拿掉了：
 * `metadata` 在解析后的技能记录里是**整份替换**而不是合并，所以覆盖过的技能会连带丢掉自己
 * frontmatter 里的 `metadata` —— 那是我复现不了的东西（本插件的 frontmatter 解析器刻意不解析
 * 嵌套映射）。写了没人读，却换掉了一个真实字段，不划算。
 *
 * 策略覆盖的信息由本插件自己的接口给出（`/overrides`），不占用注册表的字段。
 */
  ...
  return {
    ...
    list: async (options) => {
      ...
      return overridesOf(catalog)
        ...
        .map((record) => candidateFor(record))
    },
