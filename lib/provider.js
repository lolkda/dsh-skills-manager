...
export function createProvider(deps) {
  ...
  return {
    ...
    list: async (options) => {
      ...
      // 双保险：发出**一条**不合法候选的代价是整次快照降级成空清单（注册表对候选做非空描述等
      // 校验，抛了就全没了）。所以宁可在这一层也筛一遍，漏掉一条覆盖，好过模型一条技能都拿不到。
      return overridesOf(catalog)
        .filter((record) => canEmitCandidate(record))
        .map((record) => candidateFor({ ...record, metadata: metadataOf(readFile(record.docPath)) }))
    },
    get: async (candidate, options) => {
      ...
      return {
        ...
        path: docPath,
        ...optionalMetadata(metadataOf(text)),
        content: bodyOf(text),
      }
    },
  }
...
export function candidateFor(record) {
  return {
    ...
    // `whenToUse` 是技能格式里的正式字段，解析后的记录会带着它传给下游消费者。
    // 不转发的话，一条技能**只要被启停过一次**，它的 whenToUse 就会对所有消费者消失 ——
    // 启停只该改变调用策略，不该改变技能的其它性质。
...
/**
 * 仅补齐 metadata 的 YAML 对象解析，不改变当前正文/分隔符语义（R21 暂缓）。
 * @param {string|undefined} text - 已读取的技能文档
 * @returns {object|undefined} 原 metadata 对象
 */
function metadataOf(text) {
  if (typeof text !== 'string') return undefined
  try {
    const value = parseYaml(splitDocument(text).frontmatter)?.metadata
    return value && typeof value === 'object' && !Array.isArray(value) ? value : undefined
  } catch {
    // 不把 metadata 解析失败扩成新的技能可加载性裁决；原生语义对齐另行处理。
    return undefined
  }
}

/** @param {object|undefined} metadata - 原元数据 @returns {object} 可选字段 */
