 ...
 *
 * 父项是普通文件时 lstat 抛 ENOTDIR，而不是返回"不存在"：覆盖导入正是这种情形
 * —— 旧版本把附件存成文件 `data`，新版本要把它换成目录 `data/child.txt`。
 * 那是"这一层还没有"，不是解析失败，所以按"最近存在的祖先"照常推导；真正的
 * 解析失败（损坏的链接、权限不足）仍然抛出，由 `within` 拒绝。
 *
...
function canonicalDestination(path) {
  ...
  if (entry) return realpathSync.native(path)
