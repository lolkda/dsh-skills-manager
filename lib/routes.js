...
async function registryResponse(runtime, cwd, ctx) {
   ...
   * @param {object} skills - 某个上下文上的注册表
   * @returns {Promise<object>} 摘要
   */
  const summarize = async (skills) => {
    const snapshot = await skills.snapshot({ cwd: resolve })
    return {
  ...
  // 本插件的宿主层快照**不是**会话看到的那一层。真实技能由 preset 层提供，而
  // `dsh-skill` 的 collectFresh 用 `[layers.global, ...chainLayers(scope)]` 取候选：
  // 不带 scope 读到的只有 global，在真实部署里那是空的。所以这里优先读**每个 agent
  // 自己的上下文**上的注册表 —— cordis 的服务方法经过按 ctx 绑定的代理，从 agent 的
  // ctx 读到的就是该 agent 所在层链的裁决结果，也正是模型真正看到的那一份。
  ...
  for (const agent of Array.isArray(list) ? list : []) {
    ...
    if (!scoped || typeof scoped.snapshot !== 'function') continue
    try {
      perAgent.push({
        id: String(agent.id ?? agent.name ?? perAgent.length),
        ...(await summarize(scoped)),
      })
    } catch (error) {
      perAgent.push({ id: String(agent.id ?? 'agent'), error: error instanceof Error ? error.message : String(error) })
    }
  }

  const host = await summarize(skills)
  const effective = perAgent.find((item) => Array.isArray(item.skills)) ?? host
  return {
    ok: true,
    data: {
      // `skills` 是**当前最能代表会话实际所见**的那一份：有 agent 就用 agent 的，没有才退回宿主层。
      scope: perAgent.some((item) => Array.isArray(item.skills)) ? 'agent' : 'host',
