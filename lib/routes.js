 ...
 * 所有路由的共同前缀。
 *
 * 刻意**不是** `/api/dsh-skills-manager`：那个前缀被 `@michengai/dsh-skills-manager`
 * 占着。两个插件挂在同一个 profile 时，同前缀的第二次 `webServer.register` 会抛错，
 * 结果是本插件的宿主半边整个挂载失败 —— 而失败原因和它自己的代码毫无关系。
 * 前缀只是本插件两半之间的内部约定，换个独有名字没有任何代价。
 */
export const ROUTE_PREFIX = '/api/lolkda-dsh-skills-manager'
...
async function handle(req, res, runtime, trustedHosts, ctx) {
  ...
  try {
    ...
    if (req.method === 'GET' && path === `${ROUTE_PREFIX}/registry`) return json(res, 200, await registryResponse(runtime, cwd))
 ...
 */
async function registryResponse(runtime, cwd) {
  ...
  const snapshot = await skills.snapshot({ cwd: cwd ?? runtime.defaultCwd })
  return {
    ok: true,
    data: {
