...
/** 所有路由的共同前缀。 */
export const ROUTE_PREFIX = '/api/dsh-skills-manager'
...
export function installRoutes(ctx, webServer, runtime) {
  ...
  const route = webServer.register({
    kind: 'prefix',
    path: ROUTE_PREFIX,
    handler: (req, res) => handle(req, res, runtime, trustedHosts),
  })
 ...
 */
async function handle(req, res, runtime, trustedHosts) {
...
function catalogResponse(runtime, cwd) {
  ...
  return {
    ...
    data: {
      ...
      logPath: runtime.settings.log ? `${runtime.settings.dshHome}\\dsh-skills-manager.log` : null,
    },
  }
}
