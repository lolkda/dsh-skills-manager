...
async function handle(req, res, runtime, trustedHosts, ctx) {
  ...
  const cwd = requested || candidates[0] || undefined
  try {
...
async function registryResponse(runtime, cwd, ctx) {
  ...
  for (const agent of Array.isArray(list) ? list : []) {
    ...
    try {
      ...
      perAgent.push(view.resolved ? { id, agent, ...summarizeSnapshot(view) } : { id, resolved: false, reason: view.reason })
    } catch (error) {
