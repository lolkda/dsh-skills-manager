...
function dispatch(path, body, runtime, cwd, candidates) {
  ...
  switch (path) {
    ...
    case `${ROUTE_PREFIX}/skill/save`: {
      ...
      return finish(writeSkillContent({ docPath: skill.skill.docPath, rootPath: lookup.root.path, content: body.content }))
    }
    ...
    case `${ROUTE_PREFIX}/trash/restore`:
      return finish(restoreFromTrash({ dshHome: runtime.settings.dshHome, id: body.id, roots: runtime.rootsFor(cwd) }))
    case `${ROUTE_PREFIX}/trash/purge`:
      return finish(purgeFromTrash({ dshHome: runtime.settings.dshHome, id: body.id }))
...
async function registryResponse(runtime, cwd, ctx) {
  ...
  if (effective) {
    ...
    divergence = { checked: false, reason: '当前没有可用的 agent 视图，拿宿主层去比只会得到假差异' }
  }
  return {
    ok: true,
    data: {
