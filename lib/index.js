 ...
 *  2. 注册 `/dsh-skills-manager/*` 供 Web 界面读写（前缀为什么不在 `/api` 下，见 routes.js）；
...
export const Config = Schema.object({
  ...
  log: Schema.boolean().default(true).description('是否把启停与写入操作追加到 $DSH_HOME/dsh-skills-manager/dsh-skills-manager.log'),
})
...
export function createRuntime(config = {}) {
  ...
  const setEnabled = (input) => {
    ...
    if (input.enabled === null) {
      ...
      if (cleared.changed) {
        saveState(settings.dshHome, cleared.state)
        state = cleared.state
        invalidate()
    ...
    if (result.changed) {
      saveState(settings.dshHome, result.state)
      state = result.state
      invalidate()
      log(
