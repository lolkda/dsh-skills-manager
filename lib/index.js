...
export function apply(ctx, config = {}) {
  ...
  ctx.effect(() => installAgentProviders(ctx, registerOn, (detail) => runtime.log('scope-snapshot', detail)), 'skills-manager：每个 agent 作用域的覆盖提供方')
 ...
 */
async function describeScope(registry, agent) {
  ...
  return `agent ${agent.id} 的技能视图：共 ${names.length} 条 —— ${names.join('、') || '（空）'}`
}
 ...
 * @returns {() => void} 组合注销函数
 */
export function installAgentProviders(ctx, registerOn, log) {
  ...
  const install = (agent) => {
    ...
    try {
      ...
      if (log) describeScope(ctx.get?.('skills') ?? ctx.skills, agent).then(log, () => {})
    } catch {
