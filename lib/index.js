 ...
 *  2. 注册 `/api/dsh-skills-manager/*` 供 Web 界面读写；
...
/**
 * 硬依赖只有技能注册表。Web 服务、工具注册表与会话服务都用 `ctx.get()` 可选获取：
 * 这样本插件在没有 Web 服务的 profile（以及单测里）也能挂载，而不是整体挂不上。
 */
...
export function apply(ctx, config = {}) {
  ...
  if (webServer && typeof webServer.register === 'function') {
    ctx.effect(() => installRoutes(ctx, webServer, runtime), 'skills-manager：HTTP 路由')
  }

  const tools = ctx.get?.('tools')
  if (tools && typeof tools.register === 'function') {
    ctx.effect(() => installTools(ctx, tools, runtime), 'skills-manager：Agent 工具')
  }
}
