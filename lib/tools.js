 ...
 * 工具结果的统一形状。
 *
 * `render` 是**必需**的：`ctx.tools.register` 会在注册时校验
 * `output { schema, render }`，缺 `render` 直接抛错 —— 而这个错误发生在插件挂载期，
 * 会把整个 DSH profile 一起带崩（不是「这个插件不可用」，是「DSH 起不来」）。
 */
const TEXT_OUTPUT = {
  schema: { type: 'object' },
...
function renderValue(value) {
  if (value && typeof value === 'object' && value.ok === false) {
    ...
    return `失败：${value.error ?? value.code ?? '未知错误'}${diagnostics}`
  }
  let text
  try {
...
export function installTools(ctx, tools, runtime) {
  ...
  runtime.log('tools-mounted', `已注册 ${disposers.length} 个 Agent 工具：${TOOLS.map((item) => item.name).join('、')}`)
  return () => {
...
export const TOOLS = [
  ...
  },
  {
    ...
    kind: 'edit',
    description:
      'Enable or disable a skill by changing the invocation policy DSH uses for it. The skill file on disk is never modified, so the change is reversible at any time. Pass enabled=false to stop DSH from loading a skill, enabled=true to force it available even when the file itself opts out. **Omit `enabled` entirely** to clear the override and fall back to whatever the file says.',
    parameters: {
      type: 'object',
      properties: {
        ...
        enabled: {
          type: 'boolean',
          description: 'true to force the skill available, false to stop DSH from loading it. Omit to clear the override and fall back to the file.',
        },
        cwd: { type: 'string' },
      },
      // `enabled` 不是必填 —— 三种状态（启用 / 停用 / 清除）里，"清除"由**省略**表达。
      // 不能写成 `type: ['boolean', 'null']`：DSH 支持的 JSON Schema 子集里 `type` 只能是
      // 单个字符串，数组形式会被 `assertSupportedJsonSchema` 直接拒掉，而且
      // `jsonSchemaToTs` 会把整个参数渲染成 `unknown`（PTC 模式下模型看到的就是这个）。
      required: ['name'],
      additionalProperties: false,
    },
    present: (args) =>
      `${args.name} → ${typeof args.enabled === 'boolean' ? (args.enabled ? 'enable' : 'disable') : 'clear'}`,
    async run(args, runtime) {
      const catalog = runtime.catalogFor(args.cwd)
      const winner = catalog.winners.get(args.name)
      if (!winner) return { ok: false, error: `找不到名为 ${args.name} 的技能` }
      // 省略 = 清除覆盖。显式传 null 在这里也会落到同一支（旧调用方兼容），但参数表已经
      // 不允许 null，所以模型不会看见这条路径。
      const enabled = typeof args.enabled === 'boolean' ? args.enabled : null
      return runtime.setEnabled({ rootKey: winner.rootKey, name: args.name, enabled, cwd: args.cwd })
    },
  },
  {
    name: 'skills_create',
  ...
  },
  {
    ...
    kind: 'edit',
    description:
      'Permanently delete a skill by removing its files from disk. There is no trash and no undo. Use only when the user explicitly asks to delete a skill.',
